...
void Outrospection::generateField(){
    uint8_t* field = (uint8_t*) malloc(sizeof(uint8_t) * 640 * 480 * 4);

    for (int i = 0; i < 640 * 480; i++){
        field[i * 4] = field[i * 4 + 1] = field[i * 4 + 2] = field[i * 4 + 3] = abs(sin(i / 10.0)) * 256.0;
    }
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 640, 480, 0, GL_RGBA, GL_UNSIGNED_BYTE, field);
...
void Outrospection::createShaders()
{
    ...
    squaresShader = Shader("sprite", "march");

    screenShader = Shader("screen", "screen");
    crtShader    = Shader("crt"   , "march");
